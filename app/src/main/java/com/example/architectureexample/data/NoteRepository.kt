...
class NoteRepository(application: Application) {
    ...
    fun insert(note: Note) {
        val insertNoteAsynTask = InsertNoteAsyncTask(noteDao).execute(note)
    }
    ...
    fun deleteAllNotes() {
        val deleteNoteAsyncTask = DeleteAllNoteAsyncTask(noteDao).execute()
    }
    ...
    companion object {
        ...
        private class UpdateNoteAsyncTask(noteDao: NoteDao) :
            AsyncTask<Note, Unit, Unit>() {
            val noteDao = noteDao
            override fun doInBackground(vararg p0: Note?) {
                noteDao.update(p0[0]!!)
        ...
        private class DeleteNoteAsyncTask(noteDao: NoteDao) :
            AsyncTask<Note, Unit, Unit>() {
            val noteDao = noteDao
            override fun doInBackground(vararg p0: Note?) {
                noteDao.delete(p0[0]!!)
        ...
        private class DeleteAllNoteAsyncTask(noteDao: NoteDao) :
            AsyncTask<Note, Unit, Unit>() {
            val noteDao = noteDao
            override fun doInBackground(vararg p0: Note?) {
